...
dd = pd.read_csv("DIADIEMDULICH.csv", header=None).values
...
def filter(data,new_data,label): # Hàm lọc sau khi đã có kết quả dự đoán
  ...
  if (ngaynghi < 4):    # Nếu ngày nghỉ ít hơn 4 thì tiến hành lọc lần 4 để lấy các điểm dữ liệu ở gần nơi ở
    fil4 = fil3[(fil3[:,-2] > noio - 3)&(fil3[:,-2] < noio + 2)]
...
def top3(fil):
  diadiem,lan = np.unique(fil[:,-3], return_counts=True) # Xem số lần xuất hiện của các địa điểm
  sort = np.argsort(-lan) # sắp xếp theo thứ tự giảm dần
  top3 = diadiem[sort][:3]
  return top3

image_path = "vietnam.jpg"
st.image(image_path, use_column_width=True)

st.markdown("""
    <div style='text-align: center; padding: 10px;'>
        <h1>Hệ thống đề xuất địa điểm du lịch tại Việt Nam</h1>
    </div>
...
if st.button("Đề xuất"):
    if tuoi is None or gioi_tinh_so is None or thunhap is None or tinh_id is None or ngaynghi is None or chiphi is None:
